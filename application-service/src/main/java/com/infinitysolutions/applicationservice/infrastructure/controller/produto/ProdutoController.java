...
public class ProdutoController {
   ...
   @Operation(
           summary = "Listar todos os produtos (paginado)",
   ...
   public PageResult<ProdutoRespostaDTO> listarTodosProdutos(
      ...
      boolean isAdmin = authenticationUtils.isAdminOrEmployee(authentication);
      PageResult<Produto> page = listarTodosProdutos.execute(isAdmin, offset, limit);

      // Mapeia o conteúdo da página para DTO
      List<ProdutoRespostaDTO> content = page.getContent()
              .stream()
              .map(ProdutoEntityMapper::toProdutoGenericoRespostaDTO)
              .toList();

      return new PageResult<>(content, page.getTotalElements(), offset, limit);
   }
